...
def do_one_full_step_with_vacc(node_list, g, state, beta, mu, g_degrees):
    ...
    if (i_count > 0) and (s_count != 0):
        ...
        for node in nodes_order:
            ...
            if state[node] == 'R':
                pass
            elif vacc_prob <= get_vacc_probability2(node, g_degrees):
                state[node] = 'R'
            else:
                if state[node] == "I":
...
def get_vacc_probability2(node, g_degrees):
    return (0.18*np.exp(g_degrees[node])/g_degrees.shape[0])
...
print(f'lambda_c: {round(lambda_c,3)}')
...
plt.legend()
plt.title(f'Wyniki dla szczepień wykładniczych dla sieci ER: ' + r'$\lambda$ = ' + f'{round(lambd, 3)}')
plt.xlabel('t [krok]')
plt.show()
plt.close()